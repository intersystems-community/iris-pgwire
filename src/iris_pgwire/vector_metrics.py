...
class VectorMetricsCollector:
    ...
    def export_prometheus_metrics(self, stats: Dict[str, Any]) -> str:
        ...
        # Counter metrics
        metrics.append(f"# HELP vector_optimizer_total_optimizations Total number of vector query optimizations")
        metrics.append(f"# TYPE vector_optimizer_total_optimizations counter")
        metrics.append(f"vector_optimizer_total_optimizations {stats.get('total_optimizations', 0)}")

        metrics.append(f"# HELP vector_optimizer_sla_violations Total number of SLA violations")
        metrics.append(f"# TYPE vector_optimizer_sla_violations counter")
        metrics.append(f"vector_optimizer_sla_violations {stats.get('sla_violations', 0)}")

        # Gauge metrics
        metrics.append(f"# HELP vector_optimizer_sla_compliance_rate SLA compliance rate percentage")
        metrics.append(f"# TYPE vector_optimizer_sla_compliance_rate gauge")
        metrics.append(f"vector_optimizer_sla_compliance_rate {stats.get('sla_compliance_rate', 100.0)}")

        metrics.append(f"# HELP vector_optimizer_avg_transformation_time_ms Average transformation time in milliseconds")
        metrics.append(f"# TYPE vector_optimizer_avg_transformation_time_ms gauge")
        metrics.append(f"vector_optimizer_avg_transformation_time_ms {stats.get('avg_transformation_time_ms', 0)}")

        metrics.append(f"# HELP vector_optimizer_max_transformation_time_ms Maximum transformation time in milliseconds")
        metrics.append(f"# TYPE vector_optimizer_max_transformation_time_ms gauge")
        metrics.append(f"vector_optimizer_max_transformation_time_ms {stats.get('max_transformation_time_ms', 0)}")
    ...
    def export_json_metrics(self, stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'constitutional_compliance': {
                ...
                'status': 'compliant' if stats.get('sla_compliance_rate', 100.0) >= 95.0 else 'non_compliant'
            },
            'performance': {
