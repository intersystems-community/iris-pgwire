...
from .models import (
    TranslationResult, ValidationIssue, IssueSeverity, PerformanceTimer,
    ParsedConstruct, ConstructMapping
)
...
class IRISErrorHandler:
    ...
    def handle_unsupported_constructs(self, sql: str,
                                    parsed_constructs: List[ParsedConstruct],
                                    strategy: Optional[ErrorStrategy] = None) -> ErrorHandlingResult:
        """
    ...
    def _identify_unsupported_constructs(self, sql: str,
                                       parsed_constructs: List[ParsedConstruct]) -> List[UnsupportedConstruct]:
    ...
    def _check_construct_support(self, construct: ParsedConstruct, sql: str) -> Optional[UnsupportedConstruct]:
    ...
    def _apply_error_strategy(self, sql: str, unsupported_constructs: List[UnsupportedConstruct],
                            strategy: ErrorStrategy) -> ErrorHandlingResult:
    ...
    def _handle_fail_fast(self, sql: str, unsupported_constructs: List[UnsupportedConstruct]) -> ErrorHandlingResult:
    ...
    def _handle_best_effort(self, sql: str, unsupported_constructs: List[UnsupportedConstruct]) -> ErrorHandlingResult:
        ...
        for construct in unsupported_constructs:
            if construct.severity == IssueSeverity.ERROR:
                errors.append(f"Unsupported construct: {construct.construct_name} - {construct.reason.value}")
            else:
                warnings.append(f"Unsupported construct: {construct.construct_name} - {construct.reason.value}")
    ...
    def _handle_passthrough(self, sql: str, unsupported_constructs: List[UnsupportedConstruct]) -> ErrorHandlingResult:
        """Passthrough strategy - leave unsupported constructs unchanged"""
        warnings = [f"Unsupported construct passed through: {c.construct_name}" for c in unsupported_constructs]
    ...
    def _handle_substitute(self, sql: str, unsupported_constructs: List[UnsupportedConstruct]) -> ErrorHandlingResult:
    ...
    def _handle_hybrid(self, sql: str, unsupported_constructs: List[UnsupportedConstruct]) -> ErrorHandlingResult:
    ...
    def _get_fallback_mapping(self, construct: UnsupportedConstruct) -> Optional[str]:
    ...
    def _setup_error_rules(self):
        ...
        self.licensing_dependent = {
            'VECTOR_COSINE', 'VECTOR_DOT_PRODUCT', 'TO_VECTOR',
            'ML_PREDICT', 'ML_TRAIN', 'PROBABILITY'
        }
...
def handle_unsupported_constructs(sql: str, parsed_constructs: List[ParsedConstruct],
                                 strategy: Optional[ErrorStrategy] = None) -> ErrorHandlingResult:
