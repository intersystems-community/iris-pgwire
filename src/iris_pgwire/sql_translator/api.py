...
from .translator import get_translator, TranslationContext, IRISSQLTranslator
from .validator import ValidationLevel
from .models import TranslationResult, PerformanceTimer
...
class TranslationRequest(BaseModel):
    ...
    validation_level: str = Field("semantic", description="Validation level: basic, semantic, strict, exhaustive")
...
class TranslationResponse(BaseModel):
    ...
    debug_trace: Optional[Dict[str, Any]] = Field(None, description="Debug trace information")
...
class CacheInvalidationRequest(BaseModel):
    ...
    pattern: Optional[str] = Field(None, description="SQL pattern to match for selective invalidation")
...
class CacheInvalidationResponse(BaseModel):
    ...
    pattern: Optional[str] = Field(None, description="Pattern used for invalidation")
...
class SQLTranslationAPI:
    ...
    def __init__(self, translator: Optional[IRISSQLTranslator] = None):
        """
        ...
        with PerformanceTimer() as timer:
            try:
                ...
                response = TranslationResponse(
                    ...
                    debug_trace=self._format_debug_trace(result.debug_trace) if result.debug_trace else None,
        ...
        try:
            ...
            if cache_stats is None:
                raise HTTPException(
                    ...
                    detail=self._create_error_response("cache_disabled", "Translation cache is not enabled")
                )
    ...
    def invalidate_cache(self, request: CacheInvalidationRequest) -> CacheInvalidationResponse:
        ...
        try:
            ...
            if not request.confirm:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=self._create_error_response("confirmation_required", "Cache invalidation requires confirmation")
                )

            # Perform invalidation
            invalidated_count = self.translator.invalidate_cache(request.pattern)

            self.logger.info(f"Cache invalidated: {invalidated_count} entries, pattern: {request.pattern}")
    ...
    def get_api_stats(self) -> Dict[str, Any]:
        ...
        return {
            'api_stats': {
                ...
                'sla_compliance_rate': max(0.0, 1.0 - (self._sla_violations / max(self._request_count, 1)))
            },
            ...
            'constitutional_compliance': {
                ...
                'overall_compliance_status': 'compliant' if self._sla_violations == 0 else 'non_compliant'
            }
        }
    ...
    def _format_debug_trace(self, debug_trace) -> Optional[Dict[str, Any]]:
    ...
    def _create_error_response(self, error_code: str, message: str, details: Optional[str] = None) -> Dict[str, Any]:
...
def create_translation_api(translator: Optional[IRISSQLTranslator] = None) -> FastAPI:
    """
...
def get_translation_api(translator: Optional[IRISSQLTranslator] = None) -> FastAPI:
