...
from .models import (
    TranslationResult, PerformanceStats, PerformanceTimer,
    ConstructMapping, DebugTrace, ConstructType
)
from .parser import get_parser, IRISSQLParser, ParsedConstruct
from .cache import get_cache, generate_cache_key, TranslationCache
from .validator import get_validator, ValidationContext, ValidationLevel, ValidationResult, SemanticValidator
from .debug import get_tracer, DebugTracer
from .mappings import (
    get_function_registry, get_datatype_registry, get_construct_registry,
    get_document_filter_registry, get_comprehensive_stats
)
...
class IRISSQLTranslator:
    ...
    def __init__(self, enable_caching: bool = True, enable_validation: bool = True,
                 enable_debug: bool = False, max_cache_size: int = 10000):
        """
    ...
    def translate(self, context: TranslationContext) -> TranslationResult:
        ...
        with PerformanceTimer() as timer:
            ...
            try:
                    ...
                    if cache_entry:
                        ...
                        if debug_trace:
                            self.tracer.add_parsing_step(
                                trace_id, "cache_hit", context.original_sql,
                                cache_entry.translated_sql, 0.1, cache_key=cache_key
                            )
                ...
                if (is_successful and self.enable_caching and
                    context.enable_caching and cache_key):
                    self.cache.put(
                        cache_key,
                ...
                self._update_session_stats(
                    ...
                    timer.elapsed_ms,
                    cache_hit=False,
                    validation_success=translation_result.validation_result.success if translation_result.validation_result else True
                )
    ...
    def _perform_translation(self, context: TranslationContext,
                           trace_id: Optional[str], debug_trace: Optional[DebugTrace]) -> TranslationResult:
        ...
        with PerformanceTimer() as total_timer:
            ...
            if debug_trace and trace_id:
                self.tracer.add_parsing_step(
                    trace_id, "parse_constructs", context.original_sql,
                    context.original_sql, parse_timer.elapsed_ms,
                    constructs_found=len(parsed_constructs)
                )

            # Step 2: Translate each construct type
            translated_sql = self._translate_constructs(
                translated_sql, parsed_constructs, construct_mappings,
            ...
            if debug_trace and trace_id:
                self.tracer.complete_trace(
                    trace_id, translated_sql, True, total_timer.elapsed_ms
                )
    ...
    def _translate_constructs(self, sql: str, parsed_constructs: List[ParsedConstruct],
                            construct_mappings: List[ConstructMapping],
                            trace_id: Optional[str], debug_trace: Optional[DebugTrace]) -> str:
        ...
        function_constructs = (constructs_by_type[ConstructType.FUNCTION] +
                              constructs_by_type[ConstructType.SYSTEM_FUNCTION] +
                              constructs_by_type[ConstructType.JSON_FUNCTION])
        if function_constructs:
            sql = self._translate_functions(sql, function_constructs,
                                          construct_mappings, trace_id)

        # Translate data types
        if constructs_by_type[ConstructType.DATA_TYPE]:
            sql = self._translate_datatypes(sql, constructs_by_type[ConstructType.DATA_TYPE],
                                          construct_mappings, trace_id)

        # Translate SQL constructs
        if constructs_by_type[ConstructType.SYNTAX]:
            sql = self._translate_sql_constructs(sql, constructs_by_type[ConstructType.SYNTAX],
                                               construct_mappings, trace_id)

        # Translate document filters
        if constructs_by_type[ConstructType.DOCUMENT_FILTER]:
            sql = self._translate_document_filters(sql, constructs_by_type[ConstructType.DOCUMENT_FILTER],
                                                 construct_mappings, trace_id)

        return sql

    def _translate_functions(self, sql: str, function_constructs: List[ParsedConstruct],
                           construct_mappings: List[ConstructMapping],
            ...
            for construct in function_constructs:
                ...
                if mapping:
                    ...
                    if construct.parameters:
                        ...
                        if '$' in new_text:
                            new_text = self._apply_function_parameters(new_text, construct.parameters)
                        else:
                    ...
                    if trace_id and self.tracer:
                        self.tracer.add_mapping_decision(
                            trace_id, construct.original_text, [new_text],
                            new_text, mapping.confidence, f"Function mapping: {mapping.notes}"
                        )

            if trace_id and self.tracer:
                self.tracer.add_parsing_step(
                    trace_id, "translate_functions", sql, sql, timer.elapsed_ms,
                    functions_translated=len(function_constructs)
                )

        return sql

    def _translate_datatypes(self, sql: str, datatype_constructs: List[ParsedConstruct],
                           construct_mappings: List[ConstructMapping],
                           trace_id: Optional[str]) -> str:
        ...
        with PerformanceTimer() as timer:
            ...
            if trace_id and self.tracer:
                self.tracer.add_parsing_step(
                    trace_id, "translate_datatypes", sql, sql, timer.elapsed_ms,
                    datatypes_translated=len(datatype_constructs)
                )

        return sql

    def _translate_sql_constructs(self, sql: str, sql_constructs: List[ParsedConstruct],
                                construct_mappings: List[ConstructMapping],
                                trace_id: Optional[str]) -> str:
        ...
        with PerformanceTimer() as timer:
            ...
            if trace_id and self.tracer:
                self.tracer.add_parsing_step(
                    trace_id, "translate_sql_constructs", sql, translated_sql,
                    timer.elapsed_ms, constructs_translated=len(mappings)
                )

        return translated_sql

    def _translate_document_filters(self, sql: str, document_constructs: List[ParsedConstruct],
                                  construct_mappings: List[ConstructMapping],
                                  trace_id: Optional[str]) -> str:
        ...
        with PerformanceTimer() as timer:
            ...
            if trace_id and self.tracer:
                self.tracer.add_parsing_step(
                    trace_id, "translate_document_filters", sql, translated_sql,
                    timer.elapsed_ms, document_filters_translated=len(mappings)
                )
    ...
    def _validate_translation(self, context: TranslationContext, translated_sql: str,
                            construct_mappings: List[ConstructMapping],
                            trace_id: Optional[str]) -> ValidationResult:
    ...
    def _finalize_translation(self, sql: str, trace_id: Optional[str],
                            debug_trace: Optional[DebugTrace]) -> str:
        ...
        with PerformanceTimer() as timer:
            ...
            if trace_id and self.tracer:
                self.tracer.add_parsing_step(
                    trace_id, "finalize_translation", sql, sql, timer.elapsed_ms,
                    optimization="whitespace_cleanup"
                )

        return sql

    def _update_session_stats(self, session_id: Optional[str], elapsed_ms: float,
                            cache_hit: bool = False, validation_success: bool = True):
    ...
    def get_translation_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # Calculate rates
            translations_per_second = (self._total_translations / uptime_seconds
                                     if uptime_seconds > 0 else 0.0)

            avg_translation_time = (self._global_stats['total_time_ms'] / self._total_translations
                                  if self._total_translations > 0 else 0.0)

            cache_hit_rate = (self._global_stats['cache_hits'] /
                            max(self._global_stats['cache_hits'] + self._global_stats['cache_misses'], 1))
            ...
            return {
                ...
                'sla_compliance_rate': max(0.0, 1.0 - (self._sla_violations / max(self._total_translations, 1))),
                'active_sessions': len(self._sessions),
                'constitutional_compliance': {
                    'sla_requirement_ms': 5.0,
                    'violations': self._sla_violations,
                    'compliance_rate': max(0.0, 1.0 - (self._sla_violations / max(self._total_translations, 1)))
                },
                'component_stats': {
    ...
    def get_session_stats(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            session = self._sessions[session_id]
            avg_time = (session.total_time_ms / session.queries_translated
                       if session.queries_translated > 0 else 0.0)
            ...
            return {
                ...
                'cache_hit_rate': session.cache_hits / max(session.cache_hits + session.cache_misses, 1),
    ...
    @contextmanager
    def translation_session(self, session_id: Optional[str] = None):
...
def translate_sql(sql: str, session_id: Optional[str] = None,
                 enable_caching: bool = True, enable_validation: bool = True,
                 enable_debug: bool = False) -> TranslationResult:
