...
from ..models import FunctionMapping, ConstructType
...
class IRISFunctionRegistry:
    ...
    def _add_string_functions(self):
        ...
        self._mappings["%SQLSUBSTRING"].add_example(
            "%SQLSUBSTRING('hello', 2, 3)", "SUBSTRING('hello' FROM 2 FOR 3)",
    ...
    def _add_datetime_functions(self):
        ...
        self._mappings["DATEADD"].add_example(
            "DATEADD('dd', 7, '2023-01-01')", "('2023-01-01' + INTERVAL '7' DAY)",
            "Add 7 days to date"
        )
        ...
        self.add_mapping(FunctionMapping(
            ...
            parameter_mapping={"interval_type": "first", "end_date": "second", "start_date": "third"},
            confidence=0.8,
    ...
    def get_mapping(self, iris_function: str) -> Optional[FunctionMapping]:
    ...
    def get_function_categories(self) -> Dict[str, List[str]]:
        ...
        for func_name in self._mappings.keys():
            if func_name.startswith("%SQL") and any(x in func_name.upper() for x in
                ["UPPER", "LOWER", "STRING", "SUBSTRING", "LENGTH", "REPLACE", "TRIM", "PAD"]):
            ...
            elif any(x in func_name.upper() for x in
                ["ABS", "CEIL", "FLOOR", "ROUND", "SQRT", "POWER", "LOG", "EXP", "SIN", "COS", "TAN"]):
            ...
            elif any(x in func_name.upper() for x in ["COUNT", "SUM", "AVG", "MIN", "MAX", "STDEV", "VAR"]):
    ...
    def search_functions(self, pattern: str) -> List[FunctionMapping]:
        ...
        for func_name, mapping in self._mappings.items():
            if (pattern_lower in func_name.lower() or
                pattern_lower in mapping.postgresql_function.lower() or
...
def get_function_mapping(iris_function: str) -> Optional[FunctionMapping]:
