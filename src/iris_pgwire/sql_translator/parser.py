...
from .models import (
    ConstructType, SourceLocation, ParsingStep, DebugTrace,
    PerformanceTimer, TranslationError, ErrorCode
)
...
class IRISSQLParser:
    ...
    def _compile_function_patterns(self) -> Dict[str, re.Pattern]:
        ...
        sql_functions = [
            'SQLUPPER', 'SQLLOWER', 'SQLSTRING', 'SQLSUBSTRING', 'SQLLENGTH',
            'SQLREPLACE', 'SQLTRIM', 'SQLLPAD', 'SQLRPAD', 'SQLABS', 'SQLCEILING',
            'SQLFLOOR', 'SQLROUND', 'SQLSQRT', 'SQLPOWER', 'SQLLOG', 'SQLEXP',
            'SQLSIN', 'SQLCOS', 'SQLTAN', 'SQLCAST', 'SQLCONVERT', 'SQLISNULL',
            'SQLNULLIF', 'SQLCOUNT', 'SQLSUM', 'SQLAVG', 'SQLMIN', 'SQLMAX',
            'SQLSTDEV', 'SQLVAR'
        ]
        ...
        # JSON functions
        json_functions = [
            'JSON_OBJECT', 'JSON_EXTRACT', 'JSON_ARRAY', 'JSON_TABLE',
            'JSON_ARRAY_LENGTH', 'JSON_ARRAY_ELEMENTS', 'JSON_EXISTS',
    ...
    def _compile_construct_patterns(self) -> Dict[str, re.Pattern]:
        ...
        patterns['TOP_PERCENT'] = re.compile(r'\bSELECT\s+TOP\s+(\d+(?:\.\d+)?)\s+PERCENT\s+', re.IGNORECASE)
        patterns['TOP_WITH_TIES'] = re.compile(r'\bSELECT\s+TOP\s+(\d+)\s+WITH\s+TIES\s+', re.IGNORECASE)
    ...
    def parse(self, sql: str, debug_mode: bool = False) -> Tuple[List[ParsedConstruct], Optional[DebugTrace]]:
        """
        ...
        try:
            ...
            if debug_mode:
                debug_trace.add_parsing_step(
                    "parsing_complete", sql, sql, timer.elapsed_ms,
    ...
    def _identify_functions(self, sql: str, debug_trace: Optional[DebugTrace]) -> List[ParsedConstruct]:
        ...
        for func_name, pattern in self._function_patterns.items():
            ...
            for match in matches:
                ...
                document_filter_functions = {
                    'JSON_EXTRACT', 'JSON_ARRAY_LENGTH', 'JSON_ARRAY_ELEMENTS', 'JSON_EXISTS',
                    'JSON_QUERY', 'JSON_VALUE'
                }
    ...
    def _identify_system_functions(self, sql: str, debug_trace: Optional[DebugTrace]) -> List[ParsedConstruct]:
    ...
    def _identify_sql_constructs(self, sql: str, debug_trace: Optional[DebugTrace]) -> List[ParsedConstruct]:
    ...
    def _identify_data_types(self, sql: str, debug_trace: Optional[DebugTrace]) -> List[ParsedConstruct]:
        ...
        for match in matches:
            ...
            if any(type_text.upper().endswith(f) for f in ['(', 'SQLUPPER', 'SQLLOWER', 'SQLSTRING']):
                continue
    ...
    def _is_iris_data_type(self, type_text: str) -> bool:
        ...
        iris_types = {
            '%String', '%Text', '%Date', '%Time', '%TimeStamp', '%Boolean',
            '%Binary', '%List', '%ArrayOfDataTypes', '%Stream', '%Status',
            '%Oid', '%GlobalCharacterStream', '%GlobalBinaryStream'
        }

        return type_text in iris_types

    def _validate_parsing(self, sql: str, constructs: List[ParsedConstruct],
                         debug_trace: Optional[DebugTrace]):
        """Validate parsing results and add warnings if needed"""
        # Check for potential missed constructs
        if '%' in sql and not any(c.construct_type in [ConstructType.FUNCTION, ConstructType.SYSTEM_FUNCTION]
                                 for c in constructs):
            warning = "SQL contains '%' characters but no IRIS functions detected - possible parsing miss"
            if debug_trace:
                debug_trace.add_warning(warning)

        # Check for TOP clauses
        if 'TOP' in sql.upper() and not any(c.metadata.get('construct_name', '').startswith('TOP')
                                          for c in constructs):
    ...
    def get_construct_summary(self, constructs: List[ParsedConstruct]) -> Dict[str, Any]:
        ...
        for construct in constructs:
            ...
            if construct.construct_type == ConstructType.FUNCTION:
                summary['functions'].append(construct.metadata.get('function_name', construct.original_text))
            elif construct.construct_type == ConstructType.SYSTEM_FUNCTION:
                summary['system_functions'].append(construct.metadata.get('system_function_name', construct.original_text))
            elif construct.construct_type == ConstructType.SYNTAX:
                summary['syntax_constructs'].append(construct.metadata.get('construct_name', construct.original_text))
            elif construct.construct_type == ConstructType.DATA_TYPE:
                summary['data_types'].append(construct.metadata.get('type_name', construct.original_text))
        ...
        try:
            ...
            for token in parsed.flatten():
                if from_seen and token.ttype is None and isinstance(token, sqlparse.sql.Token):
                    ...
                    if token_value and not token_value.upper() in ['WHERE', 'GROUP', 'ORDER', 'HAVING', 'LIMIT']:
...
def parse_sql(sql: str, debug_mode: bool = False) -> Tuple[List[ParsedConstruct], Optional[DebugTrace]]:
