...
def prepare_simple_table():
    ...
    with engine.begin() as conn:
        ...
        conn.execute(table1.insert(), [{"name": NAME_TPL % (i,)} for i in range(100)])
...
def test_sqlalchemy_simple_inline():
    ...
    with engine.connect() as conn:
        ...
        for _ in range(ITERATIONS):
            # Existing record
            conn.execute(table1.select().where(table1.c.name == NAME_TPL % randint(0, 1000)))
            # Non-existent record
            conn.execute(table1.select().where(table1.c.name == NAME_TPL % randint(1000, 2000)))
...
def test_sqlalchemy_bulk_insert():
    ...
    with engine.begin() as conn:
        t0 = time_ns()
        # Insert 1000 records
        conn.execute(bulk_table.insert(), [{"name": NAME_TPL % (i,)} for i in range(1000)])
        times.append(time_ns() - t0)
