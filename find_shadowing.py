    ...
    def visit_FunctionDef(self, node):
        ...
        for arg in node.args.args:
            if arg.arg in self.targets and len(self.parent_stack) > 1:
                print(f"Shadowing in signature: {' -> '.join(self.parent_stack)}: arg '{arg.arg}' at line {arg.lineno}")
    ...
    def visit_Name(self, node):
        if isinstance(node.ctx, ast.Store) and node.id in self.targets:
            if len(self.parent_stack) > 1:
                print(f"Shadowing assignment: {' -> '.join(self.parent_stack)}: var '{node.id}' at line {node.lineno}")

with open('src/iris_pgwire/iris_executor.py', 'r') as f:
