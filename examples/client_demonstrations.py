...
async def demo_asyncpg(results: DemoResults):
    ...
    try:
        ...
        duration = time.time() - start
        results.add_result(
            client, "Vector Creation", vector_result is not None, duration, "Vector created"
        )
        ...
        try:
            ...
            try:
                ...
                results.add_result(
                    ...
                    False,
                    0,
                    f"IntegratedML not available: {str(e)[:50]}",
                )
        ...
        await conn.executemany("INSERT INTO demo_ml_data VALUES ($1, $2, $3, $4)", test_data)
        duration = time.time() - start
        results.add_result(
            client, "Bulk Insert (1000 rows)", True, duration, f"{len(test_data)} rows"
        )
        ...
        duration = time.time() - start
        results.add_result(
            client, "Large Result Set", len(rows) >= 1000, duration, f"{len(rows)} rows fetched"
        )
    ...
    try:
        ...
        async with conn.cursor() as cur:
            ...
            results.add_result(
                client,
                "Prepared Statement",
                result[0] == "PostgreSQL Compatible!",
                duration,
                f"Result: {result[0]}",
            )
            ...
            duration = time.time() - start
            results.add_result(
                client, "Transaction", count[0] >= 0, duration, f"Row count: {count[0]}"
            )
            ...
            try:
                ...
                results.add_result(
                    client, "COPY Protocol", False, 0, f"Not implemented: {str(e)[:50]}"
                )
...
async def demo_sqlalchemy(results: DemoResults):
    ...
    try:
        ...
        duration = time.time() - start
        results.add_result(
            client, "Aggregation Query", len(rows) > 0, duration, f"{len(rows)} groups"
        )
...
async def demo_pandas_integration(results: DemoResults):
    ...
    try:
        ...
        engine = create_async_engine("postgresql+asyncpg://test_user@127.0.0.1:5432/USER")
        ...
        async with engine.begin() as conn:
            ...
            df = await conn.run_sync(
                lambda sync_conn: pd.read_sql("SELECT * FROM demo_ml_data LIMIT 100", sync_conn)
            )
        duration = time.time() - start
        results.add_result(
            client, "DataFrame Read", len(df) > 0, duration, f"{len(df)} rows to DataFrame"
        )
        ...
        duration = time.time() - start
        results.add_result(
            client, "DataFrame Analytics", True, duration, f"Stats computed: {len(stats)} metrics"
        )
...
async def demo_performance_benchmarks(results: DemoResults):
    ...
    try:
        ...
        qps = 100 / duration
        results.add_result(
            client, "Simple Query Throughput", True, duration, f"{qps:.0f} queries/sec"
        )
        ...
        for i in range(50):
            await conn.fetchval(
                f"SELECT VECTOR_COSINE(TO_VECTOR('[{i},1,2]'), TO_VECTOR('[1,{i},3]'))"
            )
        duration = time.time() - start
        vps = 50 / duration
        ...
        results.add_result(
            client, "Large Result Set", len(rows) > 0, duration, f"{rps:.0f} rows/sec"
        )
