...
def _setup_limit_param_table(iris_connection):
    with iris_connection.cursor() as cur:
        ...
        cur.execute(
            "CREATE TABLE limit_param_meta (id INT PRIMARY KEY, name VARCHAR(255))"
        )
        cur.execute(
            "INSERT INTO limit_param_meta (id, name) VALUES (1, 'First Row')"
        )
        cur.execute(
            "INSERT INTO limit_param_meta (id, name) VALUES (2, 'Second Row')"
        )
        cur.execute(
            "INSERT INTO limit_param_meta (id, name) VALUES (3, 'Third Row')"
        )
...
def test_parameterized_offset_preserves_metadata(pgwire_client, iris_connection):
    ...
    try:
        with pgwire_client.cursor() as cur:
            cur.execute(
                "SELECT id, name FROM limit_param_meta LIMIT %s OFFSET %s", (1, 1)
            )
            row = cur.fetchone()
            assert row is not None
            assert cur.description is not None
