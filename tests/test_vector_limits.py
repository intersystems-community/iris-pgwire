    ...
    try:
        ...
        with psycopg.connect(f"host=localhost port={port} dbname=USER") as conn:
            with conn.cursor() as cur:
                ...
                create_sql = (
                    f"CREATE TABLE {table_name} (id INT, embedding VECTOR(DOUBLE, {dimensions}))"
                )
                cur.execute(create_sql)

                # Insert a test vector using parameter binding
                print("    Inserting test vector...", flush=True)
                ...
                print("    Querying with parameter...", flush=True)
...
def main():
    ...
    for port, path_name in paths:
        ...
        results_by_path[path_name] = {"max_dim": max_successful_dim, "results": results}
