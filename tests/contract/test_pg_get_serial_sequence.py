...
class MockExecutor:
    ...
    def _execute_iris_query(self, query):
        ...
        for (schema, table, column), (default, is_identity) in self.columns.items():
            ...
            table_match = (
                f"TABLE_NAME = '{table.upper()}'" in query_upper
                or f"TABLE_NAME = '{table}'" in query_upper
            )
            col_match = (
                f"COLUMN_NAME = '{column.upper()}'" in query_upper
                or f"COLUMN_NAME = '{column}'" in query_upper
            )
...
def test_serial_identity_column(catalog_handler):
    ...
    executor.add_column(IRIS_SCHEMA, "users", "id", default=None, is_identity="YES")
...
def test_serial_identity_qualified_table(catalog_handler):
    ...
    executor.add_column(IRIS_SCHEMA, "posts", "id", default=None, is_identity="YES")
...
def test_non_serial_column(catalog_handler):
    ...
    executor.add_column(IRIS_SCHEMA, "users", "name", default=None, is_identity="NO")
...
def test_sequence_name_format_simple(catalog_handler):
    ...
    executor.add_column(IRIS_SCHEMA, "customers", "customer_id", default=None, is_identity="YES")
...
def test_sequence_name_with_schema_prefix(catalog_handler):
    ...
    executor.add_column(IRIS_SCHEMA, "orders", "id", default=None, is_identity="YES")
