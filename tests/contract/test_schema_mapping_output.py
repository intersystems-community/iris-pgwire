...
These tests validate the output result transformation from IRIS '{IRIS_SCHEMA}'
...
from iris_pgwire.schema_mapper import IRIS_SCHEMA, translate_output_schema
...
class TestSchemaOutputTranslation:
    ...
    def test_table_schema_translation(self):
        """{IRIS_SCHEMA} → public in table_schema column"""
        rows = [(IRIS_SCHEMA, "users", "BASE TABLE")]
    ...
    def test_non_schema_column_unchanged(self):
        """Value '{IRIS_SCHEMA}' in non-schema column should NOT be translated"""
        rows = [(IRIS_SCHEMA,)]  # Value happens to match, but column isn't schema-related
        columns = ["some_column"]
        result = translate_output_schema(rows, columns)
        assert result[0][0] == IRIS_SCHEMA  # Not translated

    def test_schema_name_column(self):
        """schema_name column should also be translated"""
        rows = [(IRIS_SCHEMA, "some_table")]
    ...
    def test_mixed_case_schema_value(self):
        ...
        rows = [(IRIS_SCHEMA.upper(), "users")]  # IRIS might return uppercase
        columns = ["table_schema", "table_name"]
        result = translate_output_schema(rows, columns)
    ...
    def test_table_name_sqluser_unchanged(self):
        """A table named '{IRIS_SCHEMA}' should NOT have its name changed"""
        rows = [
            (IRIS_SCHEMA, IRIS_SCHEMA)
        ]  # Schema is IRIS_SCHEMA, table is also named IRIS_SCHEMA
        columns = ["table_schema", "table_name"]
        result = translate_output_schema(rows, columns)
        assert result[0][0] == "public"  # Schema translated
        assert result[0][1] == IRIS_SCHEMA  # Table name preserved

    def test_multiple_schema_columns(self):
        """Multiple schema columns in same row should all be translated"""
        rows = [(IRIS_SCHEMA, IRIS_SCHEMA, "FK reference")]
        ...
        assert result[0][1] == IRIS_SCHEMA  # Not translated (not in SCHEMA_COLUMNS)

    def test_information_schema_columns_query(self):
        """Typical information_schema.columns result"""
        rows = [
            (IRIS_SCHEMA, "users", "id", "INTEGER", "NO"),
            (IRIS_SCHEMA, "users", "name", "VARCHAR", "YES"),
            (IRIS_SCHEMA, "users", "email", "VARCHAR", "NO"),
        ]
    ...
    def test_preserves_tuple_structure(self):
        ...
        rows = [(IRIS_SCHEMA, "users", 42, True, None)]
