...
class TestIRISFunctionRegistry:
    ...
    def test_string_function_mappings(self):
        ...
        if self.registry.has_mapping('%STARTSWITH'):
            ...
            assert 'LIKE' in startswith_mapping.postgresql_function or 'starts_with' in startswith_mapping.postgresql_function
    ...
    def test_constitutional_compliance(self):
        ...
        assert len(low_confidence_functions) <= 2, f"Too many low confidence mappings found: {low_confidence_functions}"
    ...
    def test_mapping_performance(self):
        ...
        functions_to_test = [
            '%SQLUPPER', '%SQLLOWER', '%SYSTEM.Version.GetNumber',
            '%SQLSTRING', '%SQLLENGTH', '%SQLREPLACE', '%SQLTRIM'
        ]
    ...
    def test_function_mapping_data_integrity(self):
        ...
        for func_name, mapping in self.registry._mappings.items():
            ...
            assert mapping.postgresql_function is not None and mapping.postgresql_function.strip() != ''
