    ...
    try:
        with psycopg.connect(f'host=localhost port={port} dbname=USER', autocommit=True) as conn:
            with conn.cursor() as cur:
                ...
                # Verify count
                count = cur.execute('SELECT COUNT(*) FROM benchmark_vectors WHERE id >= 10000 AND id < 10010').fetchone()[0]
    ...
    try:
        with psycopg.connect(f'host=localhost port={port} dbname=USER', autocommit=True) as conn:
            with conn.cursor() as cur:
                ...
                # Verify count
                count = cur.execute('SELECT COUNT(*) FROM benchmark_vectors WHERE id >= 20000 AND id < 21000').fetchone()[0]
                rows_per_sec = 1000 / (elapsed_ms / 1000)
                print(f"  ✅ COPY inserted {count} rows in {elapsed_ms:.1f}ms ({rows_per_sec:.0f} rows/sec)")
        ...
        try:
            with psycopg.connect(f'host=localhost port={port} dbname=USER', autocommit=True) as conn:
                with conn.cursor() as cur:
                    ...
                    # Verify
                    count = cur.execute(f'SELECT COUNT(*) FROM benchmark_vectors WHERE id >= 30000 AND id < 30100').fetchone()[0]
    ...
    try:
        with psycopg.connect(f'host=localhost port={port} dbname=USER', autocommit=True) as conn:
            with conn.cursor() as cur:
                ...
                print(f"    Time: {individual_ms:.1f}ms ({num_rows / (individual_ms / 1000):.0f} rows/sec)")
