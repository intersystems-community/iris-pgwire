...
def test_copy_250_patients_to_stdout(psql_command, patients_csv_file):
    ...
    try:
        result = psql_command(
            "COPY Patients TO STDOUT WITH (FORMAT CSV, HEADER)", stdout_file=export_file
        )
...
def test_copy_to_stdout_column_list(psql_command):
    ...
    # Insert test data
    psql_command(
        "INSERT INTO Patients (PatientID, FirstName, LastName, Status) VALUES (1, 'Alice', 'Test', 'Active')"
    )
    psql_command(
        "INSERT INTO Patients (PatientID, FirstName, LastName, Status) VALUES (2, 'Bob', 'Sample', 'Discharged')"
    )
...
def test_copy_to_stdout_without_header(psql_command):
    ...
    psql_command(setup_sql)
    psql_command(
        "INSERT INTO Patients (PatientID, FirstName, LastName) VALUES (1, 'Alice', 'Test')"
    )
