...
async def test_p2_extended_protocol_simple():
    ...
    try:
        ...
        print(
            f"   SSL response: {ssl_response} ({'SSL disabled' if ssl_response == b'N' else 'SSL enabled'})"
        )
        ...
        await writer.drain()
        print("   Sent Parse message: 'SELECT $1 + $2 as sum' with 2 parameters")

        # Read ParseComplete
        parse_response = await reader.read(1024)
        parse_type = parse_response[0:1]
        print(
            f"   Parse response: {parse_type} ({'ParseComplete' if parse_type == b'1' else 'Other'})"
        )
        ...
        await writer.drain()
        print("   Sent Bind message: parameters 10 and 32")
